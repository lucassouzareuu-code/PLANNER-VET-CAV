...
with col_left:
    ...
    filtered_courses = [
        ...
        if (phase_choice == "Todas as Fases" or c.phase == phase_choice)
    ...
    if add_clicked and selected_course:
        already_added = any(
            c.code == selected_course.code for c in st.session_state.registered_courses)
        if already_added:
            st.warning(f"'{selected_course.name}' já foi adicionada.")
        else:
    ...
    st.subheader(
        f"Matérias Selecionadas ({len(st.session_state.registered_courses)})")
    for i, c in enumerate(st.session_state.registered_courses):
        row_col1, row_col2 = st.columns([4, 1])
        row_col1.write(str(c))
        if row_col2.button("🗑️", key=f"remove_{i}"):
            st.session_state.registered_courses.pop(i)
            st.rerun()
